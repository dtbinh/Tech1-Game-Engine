...
char * GetFileNameForMesh(char * MeshName);
void  GetAABBForMeshAndChilds(char * MeshName, float * ReturnValue, unsigned int Depth);
struct BoneType
{
int parent;
double posX;
	...
	if(GetMeshNameNoMessage((char *)fnChild.name().asChar()))
	{
			...
			if(GetMeshFileNameNoMessage((char *)fnChild.name().asChar()))
			{
				...
				if(GetStringLengh(&RetVal[0]))
				{
					...
					if(GetMeshType((char *)fnChild.name().asChar()) == TYPE_BBOX)
					{
						sprintf(&MeshType[0], "TYPE_BBOX");
					}else if(GetMeshType((char *)fnChild.name().asChar()) == TYPE_MESH)
					{
						sprintf(&MeshType[0], "TYPE_MESH");
					}else if(GetMeshType((char *)fnChild.name().asChar()) == TYPE_NAVMESH)
					{
						sprintf(&MeshType[0], "TYPE_NAVMESH");
					}
					PT.AddChild(MeshName, &RetVal[0], &MeshType[0]);
				}
				}
...
/*
BoneType * BonMem = new BoneType[BoneCounter]; // Сюда запишем боны
int MemoryCounter = 0;
//Теперь снова пройдемся по понам чтоб записать их в массив
MItDag SecondBoneIterator(MItDag::kDepthFirst, MFn::kTransform);
while(!SecondBoneIterator.isDone())
{


// attach the function set to the object
MFnTransform BoneTrans(SecondBoneIterator.item());

// only want non-history items
if( !BoneTrans.isIntermediateObject() ) {
  	
////-----------  BoneIterator????????????  SecondBoneIterator
if(SaveBonesToMemory(BoneIterator.item(),BonMem,MemoryCounter))
MemoryCounter++;
...
float AABBReturnValue[6];

CheckSceneRoot();

unsigned int TotalNumChildren=0;
for(int x = 0; x != PT.GetNumElements(); x++)
{
	TotalNumChildren+=PT.GetNumChilds(PT.GetElementNum(x));
}
 

DBuffer.AddData("TotalNumMeshes		");
MayaAddIntToBuffer(MeshCounter.NumMeshes + MeshCounter.NumBoxes + MeshCounter.NumNavMeshes,DBuffer);
DBuffer.AddData("\nTotalNumChildren	");
MayaAddIntToBuffer(TotalNumChildren ,DBuffer);
DBuffer.AddData("\nNumMeshes		");
MayaAddIntToBuffer(MeshCounter.NumMeshes ,DBuffer);
DBuffer.AddData("\nNumBBox			");
MayaAddIntToBuffer(MeshCounter.NumBoxes ,DBuffer);
DBuffer.AddData("\nNumNavMesh		");
MayaAddIntToBuffer(MeshCounter.NumNavMeshes ,DBuffer);
DBuffer.AddData("\n");
/////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////
...
DBuffer.AddData("\nMesh	{\n");
DBuffer.AddData("			Name		");
DBuffer.AddData(&TempExMesh->Name[0]);

if(TempExMesh->MeshType == TYPE_MESH) 
{
	if(SavedMeshHas4AnglePollys)
	{
	char ErrorMessage[255];
	sprintf(&ErrorMessage[0], "Mesh \"%s\" has 4 agle pollys. Mesh cant have 4 angle pollys." , &TempExMesh->Name[0]);
	MessageBoxA(0, &ErrorMessage[0], "Error while exporting", MB_OK);
	return status;
	}

	DBuffer.AddData("\n			Type		MESH\n");
	DBuffer.AddData("			Index		");
	MayaAddIntToBuffer(MeshesIndexer.AddBone(&TempExMesh->MeshFileName[0]) ,DBuffer);

}else if(TempExMesh->MeshType == TYPE_BBOX)
{
	if(SavedMeshHas4AnglePollys)
	{
	char ErrorMessage[255];
	sprintf(&ErrorMessage[0], "Boundig box \"%s\" has 4 agle pollys. Bounding box cant have 4 angle pollys." , &TempExMesh->Name[0]);
	MessageBoxA(0, &ErrorMessage[0], "Error while exporting", MB_OK);
	return status;
	}
	DBuffer.AddData("\n			Type		BBOX\n");
	...
	DBuffer.AddData("			Index		");
	MayaAddIntToBuffer(BBOXIndexer.AddBone(&TempExMesh->MeshFileName[0]) ,DBuffer);
	DBuffer.AddData("\n			AABB		");
//AABBReturnValue;
//float * AABB = GetMeshAABB(&TempExMesh->Name[0]);
GetAABBForMeshAndChilds(&TempExMesh->Name[0], &AABBReturnValue[0],0);
MayaAddFloatToBuffer(AABBReturnValue[0], DBuffer);
DBuffer.AddData(" ");
MayaAddFloatToBuffer(AABBReturnValue[1], DBuffer);
DBuffer.AddData(" ");
MayaAddFloatToBuffer(AABBReturnValue[2], DBuffer);
DBuffer.AddData(" ");
MayaAddFloatToBuffer(AABBReturnValue[3], DBuffer);
DBuffer.AddData(" ");
MayaAddFloatToBuffer(AABBReturnValue[4], DBuffer);
DBuffer.AddData(" ");
MayaAddFloatToBuffer(AABBReturnValue[5], DBuffer);
}else if(TempExMesh->MeshType == TYPE_NAVMESH)
{
	if(SavedMeshHas3AnglePollys)
	{
	char ErrorMessage[255];
	sprintf(&ErrorMessage[0], "NavMesh \"%s\" has 3 agle pollys. NavMesh cant have 3 angle pollys." , &TempExMesh->Name[0]);
	MessageBoxA(0, &ErrorMessage[0], "Error while exporting", MB_OK);
	return status;
	}
	...
	DBuffer.AddData("\n			Type		NAVMESH\n");
	DBuffer.AddData("			Index		");
	MayaAddIntToBuffer(NavMeshIndexer.AddBone(&TempExMesh->MeshFileName[0]) ,DBuffer);
}


//***************
if(TempExMesh->MeshType != TYPE_BBOX)
{
if(TempExMesh->ConnectedToBone)
{
ConnectedToBone = true;
DBuffer.AddData("\n			ConnectedToBone	Yes");
RBone = GetRootBoneForMesh(&TempExMesh->Name[0]);
DBuffer.AddData("\n			RootJoint	");

MayaAddFloatToBuffer(RBone->PosX, DBuffer);
DBuffer.AddData(" ");
MayaAddFloatToBuffer(RBone->PosY, DBuffer);
DBuffer.AddData(" ");
MayaAddFloatToBuffer(RBone->PosZ, DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(RBone->QuatX, DBuffer);
DBuffer.AddData(" ");
MayaAddFloatToBuffer(RBone->QuatY, DBuffer);
DBuffer.AddData(" ");
MayaAddFloatToBuffer(RBone->QuatZ, DBuffer);
DBuffer.AddData(" ");
MayaAddFloatToBuffer(RBone->QuatW, DBuffer);
DBuffer.AddData(" ");
}else
{
ConnectedToBone = false;
DBuffer.AddData("\n			ConnectedToBone	No");
DBuffer.AddData("\n			MeshMatrix	");
if(MeshHasParrents(&TempExMesh->Name[0]))
{
Matrix MatA;
	float * MatrixE = GetMeshMatrix(&TempExMesh->Name[0]);
	MatA = *(Matrix *)MatrixE;
				
MayaAddFloatToBuffer(MatrixE[0], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[1], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[2], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[3], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[4], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[5], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[6], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[7], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[8], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[9], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[10], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[11], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[12], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[13], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[14], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(MatrixE[15], DBuffer);
DBuffer.AddData(" ");
}else
{
MayaAddFloatToBuffer(TempExMesh->Matrix[0], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[1], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[2], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[3], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[4], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[5], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[6], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[7], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[8], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[9], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[10], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[11], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[12], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[13], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[14], DBuffer);
DBuffer.AddData(" ");

MayaAddFloatToBuffer(TempExMesh->Matrix[15], DBuffer);
DBuffer.AddData(" ");
}
}
}
//***********************************
if(TempExMesh->MeshType == TYPE_BBOX)
{ // потомков может иметь только  BBOX
DBuffer.AddData("\n			NumChilds	");
unsigned int NumChildren = PT.GetNumChilds(&TempExMesh->Name[0]);
char * ChName;
char * MeshType;
...
for(int z = 0; z!= NumChildren; z++)
{
		...
		DBuffer.AddData("	");
		MeshType = PT.GetChildNumType(&TempExMesh->Name[0], z);
		if(!strcmp(MeshType, "TYPE_MESH"))
		{	
			//MessageBox(0,&TempExMesh->MeshFileName[0],"mesh",MB_OK);
			MayaAddIntToBuffer( MeshesIndexer.AddBone(GetFileNameForMesh(ChName)), DBuffer);
		}else if(!strcmp(MeshType, "TYPE_BBOX"))
		{
			DBuffer.AddData("	");
...
for(int j=0;j!=TempExMesh->NumShaders ; j++)
{
if(TempExMesh->MeshType == TYPE_BBOX)
{
	DBuffer.AddData("\n			Shader	");
	DBuffer.AddData(&TempExMesh->MeshFileName[0]); // Шейдер иметь может только AABB
}else
{
	//DBuffer.AddData(&TempExMesh->ShadersUsed[j].name[0]);  Меш не может иметь шейдер.  
}

//DBuffer.AddData("	");
//MayaAddIntToBuffer(TempExMesh->ShadersUsed[j].ShaderIndex,DBuffer);
//DBuffer.AddData("	");
//MayaAddIntToBuffer(TempExMesh->ShadersUsed[j].NumFacesToRender,DBuffer);
}
if(TempExMesh->MeshType != TYPE_BBOX)
{
DBuffer.AddData("\n			FileName	");
DBuffer.AddData(&TempExMesh->MeshFileName[0]);
}
DBuffer.AddData("\n	};\n\n");
...
void SplitTwoAABBs(float AABBxMin, float AABByMin, float AABBzMin, float AABBxMax, float AABByMax, float AABBzMax, float * SplitWith)
{
	SplitWith[0] = min(SplitWith[0], AABBxMin);
    SplitWith[1] = min(SplitWith[1], AABByMin);
    SplitWith[2] = min(SplitWith[2], AABBzMin);
    SplitWith[3] = max(SplitWith[3], AABBxMin);
    SplitWith[4] = max(SplitWith[4], AABByMin);
    SplitWith[5] = max(SplitWith[5], AABBzMin);

	SplitWith[0] = min(SplitWith[0], AABBxMax);
    SplitWith[1] = min(SplitWith[1], AABByMax);
    SplitWith[2] = min(SplitWith[2], AABBzMax);
    SplitWith[3] = max(SplitWith[3], AABBxMax);
    SplitWith[4] = max(SplitWith[4], AABByMax);
    SplitWith[5] = max(SplitWith[5], AABBzMax);
}
void  GetAABBForMeshAndChilds(char * MeshName, float * ReturnValue, unsigned int Depth)
{
	cout << "\n";
...
unsigned int NumChildren = PT.GetNumChilds(MeshName);
char * ChName;
char * MeshType;
float * MeshAABB;
for(int z = 0; z!= NumChildren; z++)
{
		...
		ChName = PT.GetChildNum(MeshName, z);
		//DBuffer.AddData(ChName);
		//DBuffer.AddData("	");
		MeshType = PT.GetChildNumType(MeshName, z);
		if(!strcmp(MeshType, "TYPE_MESH"))
		{	// Если меш то сохраняем AABB Этого меша сложив его с остальными найденными AABB мешей
			MeshAABB = GetMeshAABB(ChName);
			cout << "\nMesh name: " << ChName << " AABB: " << MeshAABB[0] << " " << MeshAABB[1] << " " << MeshAABB[2] << " " << MeshAABB[3] << " " << MeshAABB[4] << " " << MeshAABB[5];
			SplitTwoAABBs(MeshAABB[0], MeshAABB[1], MeshAABB[2], MeshAABB[3], MeshAABB[4], MeshAABB[5], ReturnValue);
			cout << "\nResult AABB: " << ChName << " AABB: " << ReturnValue[0] << " " << ReturnValue[1] << " " << ReturnValue[2] << " " << ReturnValue[3] << " " << ReturnValue[4] << " " << ReturnValue[5];
			//MessageBox(0,&TempExMesh->MeshFileName[0],"mesh",MB_OK);
			//MayaAddIntToBuffer( MeshesIndexer.AddBone(GetFileNameForMesh(ChName)), DBuffer);
		}else if(!strcmp(MeshType, "TYPE_BBOX"))
		{ // Если это бокс то вызываем GetAABBForMeshAndChilds и складываем результат с остальными найденными AABB мешей
			cout << "\ncalling GetAABBForMeshAndChilds with depth =" <<Depth+1 << "\n";
			GetAABBForMeshAndChilds(ChName, ReturnValue, (Depth+1));
			
			//MessageBox(0,"mesh","mesh",MB_OK);
			//MayaAddIntToBuffer(GetBOXIndex(ChName) ,DBuffer);
		}
...
char *  GetFileNameForMesh(char * MeshName)
{
	...
	for(int i = 0; i!=SMesh.NumMeshes; i++)
	{
		if(!strcmp(TempExMesh->Name, MeshName))
		{
			return &TempExMesh->MeshFileName[0];
		}
		TempExMesh = TempExMesh->NextMesh;
	}
}
