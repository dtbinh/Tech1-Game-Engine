...
void Parenting::AddChild(char * ElementName, char * ChildName, char * ChildType)
{
char ErrorMessage[255];
OneParrentInfoEx * TempElement = ParrentInfo;
Childs * NewChild;
while(TempElement!=0)
{
	if(!strcmp(ElementName, &TempElement->MeshName[0]))
	{
		...
		memcpy(&NewChild->ParrentType[0], ChildType, strlen(ChildType)+1);
...
AddChild(ElementName, ChildName, ChildType);
}
...
char * Parenting::GetChildNumType(char * ElementName, unsigned int ChildNum)
{
	int i;
char ErrorMessage[255];
OneParrentInfoEx * TempElement = ParrentInfo;
...
while(TempElement!=0)
{
	if(!strcmp(ElementName, &TempElement->MeshName[0]))
	{
		if(ChildNum >= TempElement->NumChilds)
		{
		sprintf(&ErrorMessage[0], "GetChildNumType: unable to find child No:%d", ChildNum);
		MessageBoxA(0, &ErrorMessage[0], "Error", MB_OK);
		return "";
		}
		...
		return &TempChild->ParrentType[0];
	}
	TempElement=TempElement->NextParrentInfo;
}

sprintf(&ErrorMessage[0], "GetChildNumType: unable to find element \"%s\"", ElementName);
MessageBoxA(0, &ErrorMessage[0], "Error", MB_OK);
return "";
}